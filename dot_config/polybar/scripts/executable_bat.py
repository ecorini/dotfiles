...
ICON_CHARGING = "%{F#61C766} %{F#C4C7C5}"
ICON_EMPTY = "%{F#EC7875} %{F#C4C7C5}"
ICON_LOW = "%{F#EC7875} %{F#C4C7C5}"
ICON_MID = "%{F#FDD835} %{F#C4C7C5}"
ICON_HIGH = "%{F#61C766} %{F#C4C7C5}"
ICON_FULL = "%{F#61C766} %{F#C4C7C5}"
...
def main():
    ...
    # printing prefix
    if (ac == True):
        message = ICON_CHARGING # charging symbol
    else:
        if (int(batPercent) <= 10):
            message = ICON_EMPTY # empty battery symbol
        if (int(batPercent) <= 25):
            message = ICON_LOW # low battery symbol
        elif (int(batPercent) > 25 and int(batPercent) <= 50):
            message = ICON_MID # half battery symbol
        elif (int(batPercent) > 50 and int(batPercent) <= 75):
            message = ICON_HIGH # high battery symbol
        if (int(batPercent) > 75):
            message = ICON_FULL # full battery symbol

    if (raw):
        print(int(batPercent))
    else:
        string = message + '{0:3d}'.format(int(batPercent)) + padding
        print(string)
